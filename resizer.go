...
func resizer(buf []byte, o Options) ([]byte, error) {
	...
	image, imageType, err := loadImageWithOption(buf, o)
	if err != nil {
		return nil, err
	}
...
func loadImage(buf []byte) (*C.VipsImage, ImageType, error) {
	return loadImageWithOption(buf, Options{})
}

func loadImageWithOption(buf []byte, o Options) (*C.VipsImage, ImageType, error) {
	if len(buf) == 0 {
